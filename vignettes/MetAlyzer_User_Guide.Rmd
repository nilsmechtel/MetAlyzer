...
### Rename meta data
In the meta data, the description of the 6 extraction methods is encoded in the column "Group". For downsteam analyses, we want to rename this column into "Method" using *renameMetaData*.
```{r}
...
For further filtering and plotting, the data can be reformatted into a data frame. This allows easy plotting with e.g. ggplot2 and filtering for e.g. metabolites of interest or the quantification status. If replicates are available, some statistics is automatically calculated. For our study we can add the "Method" and "Tissue" to indicate there are replicates for these variables.
```{r}
...
For example, can filter for glutamic acid (Glu) and plot the concentration.
...
In case NAs or zeros are produced, we provide zero imputation as described in ([Gegner et al.](https://doi.org/10.1101/2021.12.16.472947)). Before, the imputation, there are 12,212 zero values in the data set.
```{r}
length(which(gg_df$Concentration == 0))
```

Now we impute the data.
```{r}
obj <- imputePlottingData(obj)
imp_gg_df <- plottingData(obj)
```

After the imputation, the lowest values is non-zero. The imputation can also performed for each metabolite separately (recommended), which we do not show for simplicity since some metabolites only show zero values.
```{r}
min(imp_gg_df$imp_Conc)
```

Raw concentrations in the object can directly be transformed using the function *transformPlottingData*. By default the data is log2 transformed and written into the colunm "transf_Conc". 
```{r}
obj <- transformPlottingData(obj)
trans_gg_df <- plottingData(obj)
head(trans_gg_df)
```
For plotting, we filter again for glutamic acid:
```{r, fig.width=7, fig.height=4.5}
...
### ANOVA
As described in ([Gegner et al.](https://doi.org/10.1101/2021.12.16.472947)), an ANOVA can be used to identify metabolites which are significantly higher in one or more methods compared to all other for each metabolite.
```{r}
