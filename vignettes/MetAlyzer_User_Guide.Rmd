...
Now we impute the data for each tissue separately.
```{r}
obj <- imputePlottingData(obj, Tissue)
imp_gg_df <- plottingData(obj)
```

After the imputation, the lowest values is non-zero. The imputation can additionally be performed for each metabolite separately (recommended), which we do not show for simplicity since some metabolites only show zero values. The imputed values are written into column "imp_Conc".
```{r}
min(imp_gg_df$imp_Conc)
```

Raw or imputed concentrations in the object can directly be transformed using the function *transformPlottingData*. By default the data is log2 transformed and written into the colunm "transf_Conc". 
```{r}
obj <- transformPlottingData(obj)
trans_gg_df <- plottingData(obj)
head(data.frame(trans_gg_df))
```
