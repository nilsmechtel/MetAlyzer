...
plot_2w_anvova <- function(two_ways_res,
                        ...
                        second_con = 'mouse')
{
  ...
  two_way_p <-
    ggplot(
      two_ways_res,
      aes(
        x = two_ways_res$Metabolites,
        y = y_val,
        color = two_ways_res$Significance
      )
    ) + geom_point() +
    theme(
  ...
  two_way_p + labs(color = "Significance") + geom_hline(yintercept = 1.3, color = "black") +
    xlab("Metabolite") + ylab("-log10 P-value")
